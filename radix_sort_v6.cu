	...
	)
{
	...
	int val = (__float_as_uint(sdata[tid]) >> sort_bit * BIT_NUM) & 0b11;
	int reordered_idx = histgram_buffer[val];

	for (int layer = 0; layer < num_layer; ++layer) {
		reordered_idx += prefixSum_buffer[val][bid + slayer_offset[layer]];
		bid /= LAYER_SIZE;
	}					
...
class RadixSorterv6 :public Sorter
{
...
public:
	...
	void sort(DEBUG_FLOAT*& datas , int data_length) override{
		...
		v6::preprocess_float<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout]);
		...
		for(int i = 0; i < BITS_LEN; ++i) {
			...
			v6::init_sort<<< numBlocks , BLOCK_SIZE >>>(dev_data[pin],
														dev_prefixSum,
														i);
			...
			for (int layer = 0; layer < num_layer; ++layer) {
				...
				v6::block_prefix_sum<<< num_prefix_sum_block, BLOCK_SIZE >>>(dev_prefixSum, 
																		layer_offset[layer], 
			...
			// Sort
			v6::reorder<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout],
													  dev_data[pin], 
													  n, 
													  dev_prefixSum, 
													  dev_histgram,
		...
		v6::postprocess_float<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout]);
