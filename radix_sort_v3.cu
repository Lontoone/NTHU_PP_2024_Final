...
{
	int tid = threadIdx.x;
	int bid = blockIdx.x * LAYER_SIZE + 2 * threadIdx.x;
	unsigned int last_1 = 0;
	unsigned int last_0 = 0;
	
	__shared__ unsigned int s1[LAYER_SIZE];
	__shared__ unsigned int s0[LAYER_SIZE];

	// Load Data
	s1[2 * tid] = prefixSum_1_buffer[bid + in_offset];
	s1[2 * tid + 1] = prefixSum_1_buffer[bid + in_offset + 1];
	s0[2 * tid] = prefixSum_0_buffer[bid + in_offset];
	s0[2 * tid + 1] = prefixSum_0_buffer[bid + in_offset + 1];

	if (tid == BLOCK_SIZE - 1) {
		last_1 = prefixSum_1_buffer[bid + in_offset + 1];
		last_0 = prefixSum_0_buffer[bid + in_offset + 1];
	}
	...
	if (tid == BLOCK_SIZE - 1) {
		if (!isLast) {
			prefixSum_1_buffer[out_offset + blockIdx.x] = s1[2 * tid + 1] + last_1;
			prefixSum_0_buffer[out_offset + blockIdx.x] = s0[2 * tid + 1] + last_0;
		} else {
			histgram_buffer[1] = s0[2 * tid + 1] + last_0;
		}
	}

	prefixSum_1_buffer[bid + in_offset] = s1[2 * tid];
	prefixSum_1_buffer[bid + in_offset + 1] = s1[2 * tid + 1];
	prefixSum_0_buffer[bid + in_offset] = s0[2 * tid];
	prefixSum_0_buffer[bid + in_offset + 1] = s0[2 * tid + 1];
}

}

class RadixSorterv3 :public Sorter
{
...
public:
	...
	void sort(DEBUG_FLOAT*& datas , int data_length) override{
		...
		v3::preprocess_float<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout]);
		...
		for(int i = 0; i < BITS_LEN; ++i) {
			...
			v3::init_sort<<< numBlocks , BLOCK_SIZE >>>(dev_data[pin],
														dev_1prefixSum,
														dev_0prefixSum,
														i);
			...
			for (int layer = 0; layer < num_layer; ++layer) {
				...
				v3::block_prefix_sum<<< num_prefix_sum_block, BLOCK_SIZE >>>(dev_1prefixSum, 
																		dev_0prefixSum, 
																		layer_offset[layer], 
			...
			// Sort
			v3::reorder<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout],
													  dev_data[pin], 
													  n, 
													  dev_1prefixSum, 
													  dev_0prefixSum, 
			...
			/*
			float t[LAYER_SIZE];
			cudaMemcpy(t, dev_data[pout], LAYER_SIZE * sizeof(float), cudaMemcpyDeviceToHost);
			printf("============================= datas =================================\n");
			...
			unsigned int temp[LAYER_SIZE];
			cudaMemcpy(temp, dev_0prefixSum, LAYER_SIZE * sizeof(int), cudaMemcpyDeviceToHost);
			for (int i = 0; i < LAYER_SIZE; ++i) printf("%u ", temp[i]);
			 */
		}
		
		
		// Copy origin_datas into sortable by index.
		v3::postprocess_float<<< numBlocks , BLOCK_SIZE >>>(dev_data[pout]);
